...
int proc_args(int argc, char **argv, map<string, string> & filenames, map<string,int> & cmdoptions, map<string,double> & d_cmdoptions, int & Niter, int & Nthin, int & Nburn)
{
    /* set the default values before parsing the command line */
    ...
    /* OK, default setting is over. Now parse the command-line */
...
double rnorm(double mu,double sigma) // normal random generator mean mu and sd sigma (Ripley (1987), alg 3.17, P82)
{
...
double rgamma(double n,double lambda) // gamma random generator from Ripley, 1987, P230
{
...
void rdirichlet(const double * a, const int k, double * b) // dirichlet random generator set b to be ~Dirichlet(a)
{
    int i;
    double sum=0.0;
    for(i=0;i<k;i++) {
...
void rperm(vector<int> & perm,int n) // Random permutation of 0 to n-1, in perm
{
    int i,s,temp,t;
    for(i=0;i<n;i++)
        perm[i]=i;
