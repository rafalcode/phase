...
int main ( int argc, char** argv)
{
    ...
    for (int dataset = 0; dataset < cmdoptions["Nskip"]; dataset++) { // Set loci types

        ClassPop allpop; // initialization and read in data
    ...
    for (int dataset = 0; dataset < cmdoptions["NDatasets"]; dataset++) { // Set loci types
        ...
        allpop.initialize ( ifstr_known, ifstr_init, cmdoptions["knowninfo"], cmdoptions["initmethod"], cmdoptions["theta"], vecDelta, cmdoptions["format"]);
        ...
        for(int count = 0; count < cmdoptions["Nrep"]; count++){
            ...
            if(cmdoptions["inferrho"]==1){ // just do MCMC on input
            ...
            } else if(cmdoptions["listresolve"]==1) { // List method
                double likelihood;  
                if(!TOOLONG) {
                ...
                if(cmdoptions["fRecom"]==0) {
                    ...
                    score = allpop.resolve_phase_NR ( Nburn, Nthin, Niter, output, monitorfile, vecDelta, cmdoptions["verbose"], cmdoptions["AncUpdate"], cmdoptions["NaiveGibbs"]);	     

                } else
                    allpop.resolve_phase_R ( d_cmdoptions["theta"], d_cmdoptions["rhostart"], Nburn, Nthin, Niter, output, monitorfile, vecDelta, cmdoptions["verbose"]);
            }
