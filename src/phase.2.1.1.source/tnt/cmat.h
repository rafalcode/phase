 ...
 *
 * $Id: cmat.h,v 1.1 2001/04/09 16:23:51 nali Exp $
 */
...
// Implementing Matrix * Scalar arithmetic
//                                       -- NL
template <class T>
inline Matrix<T> operator* ( T a, const Matrix<T> & B )
{
    Subscript M = B.num_rows();
...
template <class T>
inline Matrix<T> operator* ( const Matrix<T> & A, T b )
{
    Subscript M = A.num_rows();
    Subscript N = A.num_cols();
